...
class FailureRecovery(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        super(FailureRecovery, self).__init__(*args, **kwargs)
        self.mac_to_port = {}
        self.ip_to_mac = {}
	...
	path = [(src_dpid, src_port)] + mid_path + [(dst_dpid, dst_port)]
        self.logger.info("path : [{}-->{}]:{}".format(ip_src,ip_dst,str(path)))
    ...
    def delete_flow(self, src_ip=None, src_port=None, dst_ip=None, dst_port=None, datapath =None):
    ...
    def get_OFPPortStatus_msg(self, ev):
        ...
        desc = body.desc
        name = desc.name
        dpid = body.datapath.id
        datapath = body.datapath
        reason = body.reason
        dpidPort_to_ip = self.network_monitor.DpidPort_to_ip
        if reason == 0:
            info = ("%s has added!" % name)
        elif reason == 1:
            info = ("%s has deleted!" % name)
        elif reason == 2:
            info = ("%s has modified!" % name)
        # self.logger.info("name: %s", body)
        # self.logger.info("datapath: %s", dpid)
        port_num = self.port_name_to_num.get(name)
        key = (dpid, port_num)
        (src_ip, dst_ip) = dpidPort_to_ip.get(key, (None, None))

        (first_dpid, last_dpid) = self.ip_to_switch.get((src_ip, dst_ip), (None, None))
        if src_ip and dst_ip:
            self.delete_flow(src_ip=src_ip, dst_ip=dst_ip, datapath=datapath)
